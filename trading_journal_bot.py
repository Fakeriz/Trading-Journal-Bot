...
# Fungsi untuk menampilkan jurnal trading pengguna
...
def start(update: Update, context: CallbackContext):
    update.message.reply_text("Selamat datang di Trading Journal Bot. Anda bisa menambahkan trading, melihat jurnal, atau mengelola entri trading Anda. Gunakan /help untuk informasi lebih lanjut.")

# Fungsi untuk mencatat trading
def add_entry(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    args = context.args
    if len(args) < 8:
        update.message.reply_text("Format tidak lengkap! Harap masukkan data dalam format berikut:\n\nTanggal & Waktu, Pair, Order Type, Entry Price, TP, SL, Exit Price, Profit/Loss, [Catatan Tambahan], [Link TraderView]")
        return
    ...
    update.message.reply_text("Entry jurnal trading berhasil ditambahkan!")

# Fungsi untuk melihat jurnal trading pribadi
def view_entries(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    ...
    if rows:
        ...
        update.message.reply_text("Jurnal Anda kosong!")

# Fungsi untuk menghapus entri berdasarkan ID
def delete_entry_cmd(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    entry_id = int(context.args[0])
    delete_entry(entry_id, user_id)
    update.message.reply_text(f"Entri dengan ID {entry_id} berhasil dihapus.")

# Fungsi untuk menghapus seluruh entri jurnal pengguna
def clear_journal_cmd(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
    clear_journal(user_id)
    update.message.reply_text("Seluruh entri jurnal Anda telah dihapus.")

# Fungsi untuk export jurnal ke CSV
def export_journal(update: Update, context: CallbackContext):
    user_id = update.message.from_user.id
...
def main():
    ...
    dp.add_handler(CommandHandler("add_entry", add_entry))
    dp.add_handler(CommandHandler("view_entries", view_entries))
    dp.add_handler(CommandHandler("delete_entry", delete_entry_cmd))
    dp.add_handler(CommandHandler("clear_journal", clear_journal_cmd))
    dp.add_handler(CommandHandler("export", export_journal))
